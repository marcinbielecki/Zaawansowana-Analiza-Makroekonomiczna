...
def PlotBudgetSet(y_0=1, y_1=1, r=0.05, figsize=(5, 5), fig_ax=None, full_labels=False):
    ...
    p = ax.plot(c_0, (1+r)*(y_0-c_0) + y_1, linewidth=2, label='Ograniczenie budżetowe')
    color = p[0].get_color()

    if full_labels == True:
        ax.fill_between(c_0, 0, (1+r)*(y_0-c_0) + y_1, alpha=0.25, label='Zbiór budżetowy')
    else:
    ...
    if full_labels==True:
        plt.xticks((0, y_0, c_0_max, xmax), 
                   ('0', '$y_{1}$', '$y_{1}+y_{2}/(1+r)$', '$c_{1}$'), 
                   fontsize=12)
        plt.yticks((0, y_1, c_1_max, ymax), 
                   ('', '$y_{2}$', '$(1+r)\cdot y_{1}+y_{2}$', '$c_{2}$'), 
                   fontsize=12)
    
        ax.annotate('Nachylenie = $-(1+r)$', va='top', ha='right', 
        ...
        plt.xticks((0, y_0, xmax), ('0', '$y_{1}$', '$c_{1}$'), fontsize=12)
        plt.yticks((0, y_1, ymax), ('', '$y_{2}$', '$c_{2}$'), fontsize=12)
...
def PlotIndifferenceCurves(y_0=1, y_1=1, β=0.95, r=0.05, levels=[1], figsize=(5, 5), fig_ax=None, full_labels=False):
    ...
    for level in levels:
        ...
        if full_labels == True:
            ...
            ax.plot(c_0, np.exp((level-np.log(c_0))/β), linewidth=2, label='Krzywa obojętności')
    ...
    if full_labels == True:
        plt.xticks((0, xmax), ('0', '$c_{1}$'), fontsize=12)
...
def SolveUtilityMaximizationProblem(y_0=1, y_1=1, β=0.95, r=0.05, figsize=(5, 5)):
    ...
    p = ax.plot(c_0, β*(1+r)*c_0, linewidth=2, label='Równanie Eulera')
    color = p[0].get_color()
    ...
    if c_0_opt == y_0:
        plt.xticks((0, y_0, xmax), ('0', '$c_{1} = y_{1}$', '$c_{1}$'), fontsize=12)
        plt.yticks((0, y_1, ymax), ('', '$c_{2} = y_{2}$', '$c_{2}$'), fontsize=12)
    elif abs(c_0_opt-y_0) < xmax/20:
        if c_0_opt > y_0:
            plt.xticks((0, (y_0+c_0_opt)/2, xmax), ('0', '$c_{1} > y_{1}$', '$c_{1}$'), fontsize=12)
            plt.yticks((0, (y_1+c_1_opt)/2, ymax), ('', '$c_{2} < y_{2}$', '$c_{2}$'), fontsize=12)
        else:
            plt.xticks((0, (y_0+c_0_opt)/2, xmax), ('0', '$c_{1} < y_{1}$', '$c_{1}$'), fontsize=12)
            plt.yticks((0, (y_1+c_1_opt)/2, ymax), ('', '$c_{2} > y_{2}$', '$c_{2}$'), fontsize=12)
    else:
        plt.xticks((0, y_0, c_0_opt, xmax), ('0', '$y_{1}$', '$c_{1}$', '$c_{1}$'), fontsize=12)
        plt.yticks((0, y_1, c_1_opt, ymax), ('', '$y_{2}$', '$c_{2}$', '$c_{2}$'), fontsize=12)

    # plt.savefig('test.pdf')
    
    print("Dochód konsumenta:")
    print('  y_1: {:.4f}'.format(y_0))
    print('  y_2: {:.4f}'.format(y_1))
    print('')
    
    print('Optymalna konsumpcja:')
    print('  c_1: {:.4f}'.format(c_0_opt))
    print('  c_2: {:.4f}'.format(c_1_opt))
    print('')
    
    if c_0_opt > y_0:
        print('c_1 > y_1: Konsument się zapożycza, a < 0')
    elif c_0_opt < y_0:
        print('c_1 < y_1: Konsument oszczędza, a > 0')
    else:
        print('c_1 = y_1: Konsument ani nie oszczędza, ani się nie zapożycza, a = 0')
    print('')
...
def ShowIncomeAndSubstitutionEffects(y_0=1, y_1=1, β=0.95, r=0.05, r_new=1, figsize=(5, 5)):
    ...
    ax.plot(y_0, y_1, 'o', color='C0', label='Zasób początkowy')
    
    ax.plot(c_0_opt, c_1_opt, 'o', color='C2', markerfacecolor='none', label='Pierwotny optymalny wybór')
    ...
    ax.plot(c_0_new, c_1_new, 'o', color='C2', label='Nowy optymalny wybór')
    ...
    ax.plot(c_0_sub, c_1_sub, 'o', color='k', label='Efekt substytucyjny')
    ...
    if c_0_opt == c_0_new:
        plt.xticks((0, c_0_opt, xmax), ('0', '$c_{1} = c^{\prime}_{1}$', '$c_{1}$'), fontsize=12)
    elif abs(c_0_opt-c_0_new) < xmax/20:
        if c_0_opt > c_0_new:
            plt.xticks((0, (c_0_new+c_0_opt)/2, xmax), ('0', '$c^{\prime}_{1} < c_{1}$', '$c_{1}$'), fontsize=12)
        else:
            plt.xticks((0, (c_0_new+c_0_opt)/2, xmax), ('0', '$c^{\prime}_{1} > c_{1}$', '$c_{1}$'), fontsize=12)
    else:
        plt.xticks((0, c_0_new, c_0_opt, xmax), ('0', '$c^{\prime}_{1}$', '$c_{1}$', '$c_{1}$'), fontsize=12)
    
    if c_1_opt == c_1_new:
        plt.yticks((0, c_1_opt, ymax), ('', '$c_{2} = c^{\prime}_{2}$', '$c_{2}$'), fontsize=12)
    elif abs(c_1_opt-c_1_new) < ymax/20:
        if c_1_opt > c_1_new:
            plt.yticks((0, (c_1_new+c_1_opt)/2, ymax), ('', '$c^{\prime}_{2} > c_{2}$', '$c_{2}$'), fontsize=12)
        else:
            plt.yticks((0, (c_1_new+c_1_opt)/2, ymax), ('', '$c^{\prime}_{2} < c_{2}$', '$c_{2}$'), fontsize=12)
    else:
        plt.yticks((0, c_1_new, c_1_opt, ymax), ('', '$c^{\prime}_{2}$', '$c_{2}$', '$c_{2}$'), fontsize=12)
